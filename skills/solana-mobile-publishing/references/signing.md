...
In a prebuild-managed Expo project, `android/` is generated output: any prebuild rewrites
`build.gradle`, so a signing block added by hand disappears the next time one runs — not only on
`--clean`. Templates scaffolded by `solana-mobile create` gitignore `/android` and run prebuild
from `npm run android:build`, so a hand edit there survives exactly until the next build script.
Check whether `android/` is committed and whether `.gitignore` lists it before assuming an edit
will last.

If it is generated, pick one of these rather than re-patching by hand every time:

| Approach | When it fits |
| --- | --- |
| Let EAS hold the credentials | Default choice. No Gradle edit at all — see [Expo and EAS](#expo-and-eas) |
| A config plugin that patches `build.gradle` | Staying on prebuild but signing locally. `withAppBuildGradle` re-applies the edit on every prebuild, so it cannot drift |
| Commit `android/` and stop running prebuild | You already treat the native project as source |

Re-applying the patch by hand after each prebuild works for a one-off, but it is a step someone
eventually forgets — and forgetting it does not fail the build, it produces a debug-signed
release APK.
...
Two things about the file you are editing, before the snippet:

**Expo's generated `release` block is already signed — with the debug key.** Prebuild writes
`signingConfig signingConfigs.debug` into the release build type, under a comment telling you to
generate your own keystore. Leave it and you ship a release APK signed by `CN=Android Debug`,
which `apksigner verify` passes without complaint. The line to change is that one; adding a new
`signingConfigs` entry without replacing it changes nothing.

**Do not paste this block wholesale.** The generated file already has a `signingConfigs { debug
{ ... } }` entry and a `release` block carrying `shrinkResources`, `minifyEnabled`,
`proguardFiles` and `crunchPngs`. Add `dappStore` *alongside* the existing `debug` entry, and
inside `release` replace only the `signingConfig` line. A wholesale paste either duplicates
`signingConfigs` or silently drops Expo's release settings.
...
`apksigner` is not on `PATH` on a normal SDK install; it lives under
...
against the key you intended — and check the certificate *name* too: `CN=Android Debug` means
the signing config never took effect and the release was signed with the debug keystore. On an update this is worth doing every time — an APK signed with
...
| `apksigner` reports `CN=Android Debug` | The release variant is still on Expo's default debug `signingConfig`, or a prebuild overwrote the edit that changed it. On an update this is fatal and unrecoverable — the key differs from the previous release |
